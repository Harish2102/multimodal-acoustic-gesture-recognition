clc;
clear all;
list_data = dir(fullfile('*gain_1_dacq_COM04*.dat'));
disp(list_data(1).name);

for idx = 1:length(list_data)
    filename = list_data(idx).name;


    [h, x_volts, x_bits] = processFile(filename);   % US25
    ...
    base_name = "TimeSeriesUS_" + str_arr{end-1} + "_" + string(label(1));
